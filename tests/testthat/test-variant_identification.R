test_that("normalize_chrom adds chr prefix consistently", {
  expect_equal(ClinicalVariantR:::normalize_chrom("17"), "chr17")
  expect_equal(ClinicalVariantR:::normalize_chrom("chr17"), "chr17")
  expect_equal(ClinicalVariantR:::normalize_chrom("CHR17"), "chr17")
  expect_equal(ClinicalVariantR:::normalize_chrom("X"), "chrX")
  expect_equal(ClinicalVariantR:::normalize_chrom("MT"), "chrM")
  expect_equal(ClinicalVariantR:::normalize_chrom("chrM"), "chrM")
})

test_that("variant_key_chr_pos_ref_alt normalizes chr for matching", {
  key_with <- ClinicalVariantR:::variant_key_chr_pos_ref_alt("chr17", 41223094, "T", "G")
  key_without <- ClinicalVariantR:::variant_key_chr_pos_ref_alt("17", 41223094, "T", "G")
...
test_that("variant_key_from_parts aliases normalized key builder", {
  expect_equal(
    ClinicalVariantR:::variant_key_from_parts("X", 154030906, "T", "C"),
    "chrX:154030906:T:C"
  )
})

test_that("variant_coords_equal matches chr and no-chr coordinates", {
  expect_true(ClinicalVariantR:::variant_coords_equal("chr17", 1, "A", "G", "17", 1, "A", "G"))
  expect_false(ClinicalVariantR:::variant_coords_equal("chr17", 1, "A", "G", "chr17", 2, "A", "G"))
})

test_that("split_vcf_alt_alleles expands multi-allelic ALT fields", {
  expect_equal(ClinicalVariantR:::split_vcf_alt_alleles("A,G,T"), c("A", "G", "T"))
  expect_equal(ClinicalVariantR:::split_vcf_alt_alleles("A"), "A")
  expect_equal(ClinicalVariantR:::split_vcf_alt_alleles("."), ".")
})
...
test_that("parse_vcf_line expands multi-allelic records", {
  ...
  parsed <- ClinicalVariantR:::parse_vcf_line(line, NULL)
...
test_that("find_reference_hits matches PS4 DB with or without chr prefix", {
  ...
  db$chrom <- ClinicalVariantR:::normalize_chrom(db$chrom)
  db$variant_key <- ClinicalVariantR:::variant_key_chr_pos_ref_alt(db$chrom, db$pos, db$ref, db$alt)
  ...
  hits_chr <- ClinicalVariantR:::find_reference_hits(row_chr, db)
  hits_no_chr <- ClinicalVariantR:::find_reference_hits(row_no_chr, db)
...
test_that("find_reference_hits falls back to rsID when genomic key misses", {
  ...
  hits <- ClinicalVariantR:::find_reference_hits(row, db)
...
test_that("parse_protein_change identifies PS1/PM5 protein coordinates", {
  change <- ClinicalVariantR:::parse_protein_change("p.Thr320Ala", NA_character_, NA_character_)
  expect_equal(change$protein_position, 320L)
  expect_equal(change$ref_aa, "THR")
  expect_equal(change$alt_aa, "ALA")

  from_aa_field <- ClinicalVariantR:::parse_protein_change(
...
test_that("annotate_variants joins reference rows across chr conventions", {
  ...
  for (ref_name in names(refs)) {
    data.table::set(refs[[ref_name]], j = "chrom", value = ClinicalVariantR:::normalize_chrom(refs[[ref_name]]$chrom))
    data.table::set(
      refs[[ref_name]],
      j = "variant_key",
      value = ClinicalVariantR:::variant_key_chr_pos_ref_alt(
        refs[[ref_name]]$chrom,
        refs[[ref_name]]$pos,
        refs[[ref_name]]$ref,
        refs[[ref_name]]$alt
  ...
  annotated <- ClinicalVariantR:::annotate_variants(variants, refs)
...
test_that("match_variant_rows selects normalized coordinates from parsed table", {
  ...
  hit <- ClinicalVariantR:::match_variant_rows(parsed, "1", 100, "A", "T")
