...
register_explorer_server <- function(ctx) {
  ...
  output$download_explorer_pdf <- downloadHandler(
    filename = function() {
      ...
      paste0("ACMGamp_Evidence_Report_", src, "_", scope, "_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf")
    },
  ...
  output$download_explorer_csv <- downloadHandler(
    filename = function() {
      ...
      paste0("ACMGamp_Evidence_Report_", src, "_", scope, "_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".csv")
    },
    content = function(file) {
      scope <- isolate(input$explorer_pdf_scope) %||% "all"
