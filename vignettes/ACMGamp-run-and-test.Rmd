...
It complements the [Introduction to ClinicalVariantR](ACMGamp.html) vignette.

# Prerequisites

ClinicalVariantR dependencies are declared in `DESCRIPTION` and are installed by
the standard package build/check workflow. The chunk below verifies that the
main runtime dependencies are available in the current build environment:

```{r dependency-check}
deps <- c("shiny", "bslib", "DT", "data.table", "readr", "jsonlite", "VariantAnnotation")
vapply(deps, requireNamespace, quietly = TRUE, FUN.VALUE = logical(1))
```
...
```{r unit-tests}
if (FALSE) {
# After local installation or during development:
...
system2("Rscript", "scripts/audit_acmg_rules.R")

# Group B/C coverage and gene-filter parity
system2("Rscript", "scripts/verify_group_b_c.R")

# Group A 28-criterion catalog check
system2("Rscript", "scripts/verify_group_a_28.R")

# Score the Group B benchmark VCF vs expected labels
system2("Rscript", "scripts/run_group_b_benchmark.R")

# Smaller accuracy fixture
system2("Rscript", "scripts/run_accuracy_benchmark.R")

# Formal validation report (expert set)
system2("Rscript", "scripts/generate_validation_report.R")
}
```
...
```{r upload-flow}
if (FALSE) {
system2("Rscript", "scripts/test_upload_flow.R")
}
```
