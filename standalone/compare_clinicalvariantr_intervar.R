#!/usr/bin/env Rscript
# Standalone: compare ClinicalVariantR CSV vs InterVar output or InterVar-style reference TSV.
...
#
# Usage:
#   Rscript compare_clinicalvariantr_intervar.R --clinicalvariantr report.csv --reference sample.acmg.tsv
#   Rscript compare_clinicalvariantr_intervar.R --clinicalvariantr report.csv --intervar myanno.hg38_multianno.txt.intervar
#   Rscript compare_clinicalvariantr_intervar.R --clinicalvariantr report.csv --reference ref.tsv --out results/compare_042
#
# Optional: run ClinicalVariantR first from project root:
#   Rscript scripts/benchmark_testing_dataset.R ../testig/testig ../results/benchmark

args <- commandArgs(trailingOnly = TRUE)

parse_args <- function(args) {
  out <- list(clinicalvariantr = NA_character_, reference = NA_character_, intervar = NA_character_, out = NA_character_)
  i <- 1L
  while (i <= length(args)) {
    key <- args[[i]]
  if (key %in% c("--clinicalvariantr", "-c") && i < length(args)) {
      out$clinicalvariantr <- args[[i + 1L]]; i <- i + 2L
      ...
      cat(
        "Compare ClinicalVariantR vs InterVar or reference TSV\n\n",
        "Required: --clinicalvariantr FILE\n",
...
if (is.na(opts$clinicalvariantr)) stop("Provide --clinicalvariantr path to ClinicalVariantR CSV report.")
...
acmg_df <- load_clinicalvariantr_report_csv(normalizePath(opts$clinicalvariantr, winslash = "/", mustWork = TRUE))
...
result <- compare_two_classification_tables(acmg_df, ref_df, left_name = "ClinicalVariantR", right_name = ref_name)
...
cat("=== ClinicalVariantR vs ", ref_name, " ===\n", sep = "")
