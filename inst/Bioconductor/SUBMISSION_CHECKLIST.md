...
| Item | Path | Status |
|------|------|--------|
| Package metadata | `DESCRIPTION` | ✅ `Version: 0.99.4`, `biocViews`, MIT |
...
R CMD check ClinicalVariantR_0.99.4.tar.gz
# Bioconductor-specific:
BiocCheck::BiocCheckGitClone("ClinicalVariantR")
BiocCheck::BiocCheck("ClinicalVariantR_0.99.4.tar.gz", `new-package` = TRUE)
```
...
6. Bump `0.99.z` for each rebuild after fixes (current: **0.99.4**).  
